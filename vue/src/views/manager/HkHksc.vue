<template>
  <div>
		...
		<div class="card" style="margin-bottom: 5px">
			...
			<el-input v-model="data.search.generate" prefix-icon="Search" style="width: 240px; margin-right: 10px; margin-bottom: 3px" placeholder="请输入是否生成"></el-input>
			<el-input v-model="data.search.hkUrl" prefix-icon="Search" style="width: 240px; margin-right: 10px; margin-bottom: 3px" placeholder="请输入贺卡链接"></el-input>
			...
			<el-upload v-if="data.user.role === 'admin'" :action="'http://localhost:9090/hkHksc/upload?token=' + data.user.token" style="display: inline-block" :show-file-list="false" :on-success="impSuccessUpload">
    ...
    <div class="card" style="margin-bottom: 5px;">
      <el-table :data="data.tableData" strip @selection-change="handleSelectionChange">
				...
				<el-table-column label="是否生成" prop="generate"></el-table-column>
    ...
    <el-dialog title="请填写信息" v-model="data.formVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="data.form" label-width="80px"  style="padding: 20px 30px">
				...
				<el-form-item label="是否生成" prop="generate">
					<el-radio-group v-model="data.form.generate">
						<el-radio label="成功"></el-radio>
						<el-radio label="失败"></el-radio>
						<el-radio label="未生成"></el-radio>
					</el-radio-group>
				</el-form-item>
...
// 导出
const exp = () => {
	location.href = 'http://localhost:9090/hkHksc/export?token=' + data.user.token
}
