<template>
  <div>
		...
		<div class="card" style="margin-bottom: 5px">
			<el-input v-model="data.search.studetid" prefix-icon="Search" style="width: 240px; margin-right: 10px; margin-bottom: 3px" placeholder="请输入学号"></el-input>
			...
			<el-input v-model="data.search.studentsTeacher" prefix-icon="Search" style="width: 240px; margin-right: 10px; margin-bottom: 3px" placeholder="请输入辅导员"></el-input>
			...
			<el-input v-model="data.search.telephoneNumber" prefix-icon="Search" style="width: 240px; margin-right: 10px; margin-bottom: 3px" placeholder="请输入发送手机号"></el-input>
			<el-button type="info" plain @click="load">查询</el-button>
			<el-button type="warning" plain style="margin-right: 10px" @click="reset">重置</el-button>

			<el-upload v-if="data.user.role === 'admin'" :action="'http://localhost:9090/zhTodaybirthday/upload?token=' + data.user.token" style="display: inline-block" :show-file-list="false" :on-success="impSuccessUpload">
    ...
    <div class="card" style="margin-bottom: 5px;">
      <el-table :data="data.tableData" strip @selection-change="handleSelectionChange">
				...
				<el-table-column label="学号" prop="studetid"></el-table-column>
				<el-table-column label="姓名" prop="studentName"></el-table-column>
				<el-table-column label="班级" prop="studentClass"></el-table-column>
				<el-table-column label="辅导员" prop="studentsTeacher"></el-table-column>
				<el-table-column label="是否发送成功" prop="sentSuccessfully"></el-table-column>
				<el-table-column label="发送手机号" prop="telephoneNumber"></el-table-column>
				<el-table-column label="贺卡链接" prop="hkUrl"></el-table-column>
    ...
    <el-dialog title="请填写信息" v-model="data.formVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="data.form" label-width="80px"  style="padding: 20px 30px">
				<el-form-item label="学号" prop="studetid">
				...
				<el-form-item label="辅导员" prop="studentsTeacher">
					<el-select style="width: 100%" v-model="data.form.hk_studentId">
				...
				<el-form-item label="是否发送成功" prop="sentSuccessfully">
					<el-radio-group v-model="data.form.sentSuccessfully">
						<el-radio label="成功"></el-radio>
						<el-radio label="失败"></el-radio>
						<el-radio label="未发送"></el-radio>
					</el-radio-group>
				</el-form-item>
				<el-form-item label="发送手机号" prop="telephoneNumber">
					<el-select style="width: 100%" v-model="data.form.hk_studentId">
						<el-option v-for="item in data.hk_studentData" :value="item.id" :label="item.telephoneNumber" :key="item.id"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="贺卡链接" prop="hkUrl">
					<el-input v-model="data.form.hkUrl" placeholder="请输入贺卡链接"></el-input>
				</el-form-item>
      </el-form>
      <template #footer>
...
// 导出
const exp = () => {
	location.href = 'http://localhost:9090/zhTodaybirthday/export?token=' + data.user.token
}
